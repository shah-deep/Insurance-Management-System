...
<form action = "../includes/changePlan-inc.php" method = "post">
   ...
   <br><br>
   <h7>   Mode:  </h7>
   <input type="checkbox" name="Mode_Yearly" <?php if($row['MODE_YEARLY']==1){echo 'checked';} ?> value=1>
   <label for="Mode_Yearly">Yearly</label>
   <input type="checkbox" name="Mode_Halfly" <?php if($row['MODE_HALFLY']==1){echo 'checked';} ?> value=1>
   <label for="Mode_Halfly">Halfly</label>
   <input type="checkbox" name="Mode_Quartely" <?php if($row['MODE_QUARTELY']==1){echo 'checked';} ?> value=1>
   <label for="Mode_Quartely">Quartely</label>
   <input type="checkbox" name="Mode_Monthly" <?php if($row['MODE_MONTHLY']==1){echo 'checked';} ?> value=1>
   <label for="Mode_Monthly">Monthly</label>
   <input type="checkbox" name="Mode_Single" <?php if($row['MODE_SINGLE']==1){echo 'checked';} ?> value=1>
   <label for="Mode_Single">Single Premium</label>
   <br><br>
   <h7>   Type of Term:  </h7>
   <input type="radio" id="Range" name="Type_term" <?php if($row['Type_term']==0){echo 'checked';} ?> value=0>
   <label for="Range">Range</label>
   <input type="radio" id="Specific" name="Type_term" <?php if($row['Type_term']==1){echo 'checked';} ?> value=1>
